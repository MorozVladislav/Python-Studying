...
from api_tests_config import APITestsConfig
...
class TestGitHubAPI(object):
    ...
    def teardown_method(self, _):
        try:
            ...
            pass
        try:
            self.api.delete_repo(APITestsConfig.REPO_NAME)
        except UnexpectedStatusCode:
            pass

    def test_unauthorized_request(self):
        client = HttpClient(host=APITestsConfig.HOST)
    ...
    def test_create_and_delete_repo(self):
        ...
        repo_properties = {'description': APITestsConfig.REPO_DESCRIPTION}
        resp = self.api.create_repo(APITestsConfig.REPO_NAME, repo_properties)
        assert resp.json()['name'] == APITestsConfig.REPO_NAME
        assert resp.json()['description'] == APITestsConfig.REPO_DESCRIPTION
    ...
    def test_get_user_repos(self):
        ...
        repos_count = len(self.api.get_repos(params=params).json())
        repo_properties = {'description': APITestsConfig.REPO_DESCRIPTION}
        self.api.create_repo(APITestsConfig.REPO_NAME, repo_properties)
        resp = self.api.get_user_repos(self.api.username)
        assert resp.json()[repos_count]['name'] == APITestsConfig.REPO_NAME
        assert resp.json()[repos_count]['description'] == APITestsConfig.REPO_DESCRIPTION
        params = {'sort': 'created', 'direction': 'desc'}
        assert self.api.get_user_repos(self.api.username, params=params).json()[0]['name'] == APITestsConfig.REPO_NAME
    ...
    def test_edit_repo(self):
        ...
        self.api.edit_repo(APITestsConfig.REPO_NAME, repo_properties)
        resp = self.api.get_repos(params=params)
    ...
    def test_get_repo_topics(self):
        ...
        assert self.api.get_repo_topics(APITestsConfig.REPO_NAME, headers=headers).json()['names'] == []
